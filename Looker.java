...
public class Looker {	
	...
	public Looker(int x, int y, int z,double angleX, double angleY, double fovX, double fovY){
		this.x = x;
		this.y = y;
		this.z = z;
		this.angleX = angleX;
	...
	public void paintArrow(Graphics g){
		...
		int[] xPoints = {x,(int)( x + 2000*Math.cos(angleX + fovX/2)),(int)( x + 2000*Math.cos(angleX - fovX/2))};
		int[] yPoints = {y,(int)( y- 2000*Math.sin(angleX+fovX/2)),(int)( y- 2000*Math.sin(angleX -fovX/2))};
		g.fillPolygon(new Polygon(xPoints,yPoints,3));
		g.setColor(Color.BLUE);
		g.drawLine(x, y, (int) (x + 100*Math.cos(angleX)), (int) (y - 100*Math.sin(angleX)));
		g.drawLine(x, y,(int)( x + 50*Math.cos(angleX + fovX/2)),(int)( y- 50*Math.sin(angleX+fovX/2)));
		g.drawLine(x, y,(int)( x + 50*Math.cos(angleX - fovX/2)),(int)( y- 50*Math.sin(angleX -fovX/2)));
	}
	public void update(){
		moveX();
